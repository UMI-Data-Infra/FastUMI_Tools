...
1. 将相机直接连接到电脑的 USB 3.x 接口；刷新固件时只连接一台 FastUMI 相机。
2. 从应用菜单启动 **FastUMI Tools**，不要直接以 root 用户运行浏览器。
3. 在“SDK 和固件”页面确认相机序列号、USB 连接速率、实际 SDK 版本和相机报告的固件版本。
4. 如需安装 SDK 或刷新固件，请选择目标版本并按页面提示完成检查。刷新过程中不要拔线、断电、关闭服务或启动其他相机程序。
5. 操作完成后，以页面重新读取到的实际版本为准；仅有任务记录而没有设备版本验真，不会显示为刷新成功。
